      ...
      <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 bg-white rounded-2xl border shadow-sm overflow-auto">
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-slate-50 bg-white">
                {isLoading ? (
                  <tr><td colSpan={9} className="py-32 text-slate-400 animate-pulse">読み込み中...</td></tr>
                ) : currentProperties.length > 0 ? (
                  currentProperties.map((p) => (
                    <tr
                      key={p.id}
                      className={`group cursor-pointer transition-colors ${selectedId === p.id ? "bg-orange-50/70" : "hover:bg-orange-50/30"}`}
                    ...
                    >
                      ...
                      <td className="px-4 text-green-600 font-bold">{p.assumedYield}</td>
                  ...
                  {form.buyCostItems.map((item) => (
                    <div key={item.id} className="flex gap-2">
                      <input value={item.label} onChange={(e) => {
                        const next = form.buyCostItems.map((x) => x.id === item.id ? { ...x, label: e.target.value } : x);
                        setForm({ ...form, buyCostItems: next });
                      }} placeholder="項目名" className="flex-1 p-2 rounded-lg border" />
                      <input value={item.amount} onChange={(e) => {
                        const next = form.buyCostItems.map((x) => x.id === item.id ? { ...x, amount: e.target.value.replace(/[^0-9]/g, "") } : x);
                        setForm({ ...form, buyCostItems: next });
                      }} placeholder="金額" className="w-24 p-2 rounded-lg border text-right" />
                    </div>
                  ))}
                </div>
        ...
        {modalTarget && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-white rounded-3xl p-8 shadow-2xl animate-in zoom-in duration-150 text-left">
