          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-[640px] max-w-[95vw] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              <form onSubmit={upsertProperty} className="flex-1 overflow-auto p-8 space-y-6">
                <FieldZenkaku label="物件名（必須）" value={form.name} onChange={(v) => setForm({ ...form, name: toZenkakuLoose(v) })} placeholder="例）アキサスハイツ東京" required />
                <FieldHankakuNumber label="物件価格（円）" value={form.propertyPriceYen} onChange={(v) => setForm({ ...form, propertyPriceYen: toHankakuNumberOnly(v) })} placeholder="例）78000000" />
                ...
                <FieldHankakuNumber label="想定家賃（円）" value={form.assumedRentYen} onChange={(v) => setForm({ ...form, assumedRentYen: toHankakuNumberOnly(v) })} />
                <FieldHankakuNumber label="客付け家賃（円）" value={form.customerRentYen} onChange={(v) => setForm({ ...form, customerRentYen: toHankakuNumberOnly(v) })} />
                <FieldHankakuNumber label="想定販売価格（円）" value={form.expectedSalePriceYen} onChange={(v) => setForm({ ...form, expectedSalePriceYen: toHankakuNumberOnly(v) })} />
