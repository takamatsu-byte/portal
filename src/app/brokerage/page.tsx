            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-slate-50 bg-white">
                {currentProperties.length > 0 ? currentProperties.map((p) => (
                  <tr key={p.id} className={`group cursor-pointer transition-colors ${selectedId === p.id ? "bg-orange-50/70" : "hover:bg-orange-50/30"}`} onClick={() => setSelectedId(p.id)}>
                    ...
                    <td className="px-4">{formatNumber(p.projectTotal)}</td><td className="px-4">{formatNumber(p.assumedRent)}</td>
                    <td className="px-4 text-green-600 font-bold">{p.assumedYield}</td><td className="px-4">{formatNumber(p.customerRent)}</td>
                    <td className="px-4">{p.surfaceYield}</td><td className="px-4">{formatNumber(p.expectedSalePrice)}</td>
