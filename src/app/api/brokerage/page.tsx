    ...
    async function fetchData() {
      ...
      try {
        ...
        const format = (list: any[]) => list.map((p: any) => ({
          id: p.id, name: p.code, projectTotal: p.projectTotal || 0,
          assumedRent: p.expectedRent || 0, assumedYield: p.expectedYieldBp ? `${p.expectedYieldBp.toFixed(1)}%` : "-",
          ...
          propertyPrice: p.propertyPrice || 0, buyCostTotal: p.acquisitionCost || 0,
          buyCostBreakdown: p.expenses?.map((e: any) => ({ label: e.name, amount: formatNumber(e.price) }))
        }));

        setInvestmentData(format(await invRes.json()));
        setBrokerageData(format(await brokRes.json()));
        setResaleData(format(await resalRes.json()));
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans text-slate-600 bg-slate-50">
      <aside className="w-64 text-white flex flex-col flex-shrink-0" style={{ backgroundColor: BRAND_ORANGE }}>
        ...
        <nav className="p-4"><div className="bg-white text-[#FD9D24] p-3 rounded-xl font-bold text-center shadow-sm">物件管理</div></nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white h-28 border-b flex items-center justify-end px-8">
          <div className="inline-flex items-center rounded-full border border-slate-200 bg-white shadow-sm h-12 overflow-hidden">
            <button onClick={() => setIsPanelOpen(true)} className="px-8 h-full font-bold text-slate-700 hover:text-white hover:bg-[#FD9D24] transition-all border-r">＋ 追加</button>
            <button disabled className="px-8 h-full font-bold text-slate-300 transition-all border-r">編集</button>
            <button disabled className="px-8 h-full font-bold text-slate-300 transition-all">削除</button>
          </div>
        </header>
        ...
        <div className="flex-1 p-8 flex flex-col overflow-hidden text-center">
          <div className="flex justify-center mb-8">
            <div className="inline-flex bg-slate-200 p-1 rounded-full shadow-inner">
              {["仲介", "転売", "収益"].map((tab) => (
                <button key={tab} onClick={() => { setActiveTab(tab as any); setSelectedId(null); }} className={`px-12 py-3 rounded-full text-sm font-bold transition-all ${activeTab === tab ? "bg-white text-slate-800 shadow-md" : "text-slate-500 hover:text-slate-700"}`}>{tab}</button>
              ))}
            </div>
          </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {currentProperties.length > 0 ? currentProperties.map((p) => (
                  <tr key={p.id} className={`cursor-pointer transition-colors ${selectedId === p.id ? "bg-orange-50/70" : "bg-white hover:bg-orange-50/30"}`} onClick={() => setSelectedId(p.id)}>
                    <td className="px-6 py-6 text-left sticky left-0 bg-inherit border-r border-slate-100/50 font-bold text-slate-800 whitespace-nowrap">{p.name}</td>
                    <td>{formatNumber(p.projectTotal)}</td><td>{formatNumber(p.assumedRent)}</td>
                    <td className="text-green-600 font-bold">{p.assumedYield}</td><td>{formatNumber(p.customerRent)}</td>
                    <td>{p.surfaceYield}</td><td>{formatNumber(p.expectedSalePrice)}</td>
                    <td>{formatNumber(p.propertyPrice)}</td>
                    <td className="font-bold select-none px-4" onDoubleClick={() => setModalTarget(p)}>{formatNumber(p.buyCostTotal)}</td>
                  </tr>
                )) : (
                  <tr><td colSpan={9} className="py-32 text-slate-300 italic font-bold">データがありません</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        {isPanelOpen && (
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-[500px] bg-white h-full shadow-2xl p-8 flex flex-col">
              ...
              <form onSubmit={handleSave} className="space-y-6 flex-1 overflow-auto text-left">
                ...
                <label className="block space-y-2"><span className="text-xs font-bold text-slate-400">販売価格</span><input value={form.expectedSalePrice} onChange={(e) => setForm({...form, expectedSalePrice: e.target.value.replace(/[^0-9]/g, '')})} className="w-full border-2 p-4 rounded-xl font-bold text-right"/></label>
            ...
            <div className="relative w-full max-w-md bg-white rounded-3xl p-8 shadow-2xl animate-in zoom-in duration-150">
              ...
              <div className="space-y-4 mb-8">
                {modalTarget.buyCostBreakdown?.map((b, i) => (
                  <div key={i} className="flex justify-between font-bold"><span className="text-slate-400">{b.label}</span><span>{b.amount}</span></div>
                ))}
              </div>
