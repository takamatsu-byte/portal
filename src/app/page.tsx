...
type DbExpense = { id: string; name: string; price: number };
type DbProject = {
  id: string;
  createdAt: string;
  updatedAt: string;
  ...
  expectedYieldBp: number | null;

  agentRent: number | null;
  surfaceYieldBp: number | null;
...
function formatPercentFromBp(bp: number | null, digits = 1): string {
  if (bp === null || !Number.isFinite(bp)) return "-";
  const percent = bp / 100; // 920 -> 9.2%
...
function ensureTrailingEmptyCostRow(items: CostItem[]): CostItem[] {
  ...
  if (!trimmed.length || lastHasValue) trimmed.push({ id: uid(), label: "", amount: "" });
...
function dbToUi(p: DbProject): Property {
  const breakdown = (p.expenses ?? []).map((e) => ({
    label: e.name ?? "-",
    amount: formatYen(e.price ?? 0),
  }));

  return {
    id: p.id,
    name: p.propertyAddress ? p.propertyAddress : p.code || "物件",

    projectTotalYen: formatYen(p.projectTotal),
    assumedRentYen: formatYen(p.expectedRent),
    assumedYield: formatPercentFromBp(p.expectedYieldBp, 1),

    customerRentYen: formatYen(p.agentRent),
    surfaceYield: formatPercentFromBp(p.surfaceYieldBp, 1),

    expectedSalePriceYen: formatYen(p.expectedSalePrice),
    propertyPriceYen: formatYen(p.propertyPrice),

    buyCostTotalYen: formatYen(p.acquisitionCost ?? 0),
    buyCostBreakdown: breakdown.length ? breakdown : undefined,
  };
}

async function apiGetProjects(): Promise<DbProject[]> {
  const r = await fetch("/api/projects", { cache: "no-store" });
  if (!r.ok) throw new Error("一覧取得に失敗しました");
  return r.json();
}

async function apiCreateProject(payload: any): Promise<DbProject> {
...
async function apiUpdateProject(id: string, payload: any): Promise<DbProject> {
...
export default function RealEstateDashboard() {
  ...
  async function refresh() {
    setLoading(true);
    try {
      const db = await apiGetProjects();
      const ui = db.map(dbToUi);
      setProperties(ui);
      if (ui.length && !selectedId) setSelectedId(ui[0].id);
    } finally {
      setLoading(false);
    }
  }
  ...
  function openEdit() {
    ...
    setForm({
      ...
      buyCostItems: ensureTrailingEmptyCostRow(
        breakdown.length ? breakdown : [{ id: uid(), label: "", amount: "" }]
      ),
    });
  ...
  async function onDelete() {
    ...
    await apiDeleteProject(selectedId);
    setSelectedId(null);
    await refresh();
  }
  ...
  async function upsertProperty(e: React.FormEvent) {
    ...
    const expenses = form.buyCostItems
      .map((x) => ({
        name: (x.label ?? "").trim(),
        price: parseYen(x.amount) ?? 0,
      }))
      .filter((x) => x.name || x.price);

    const payload = {
      // DB側では address/code を使ってるので、ここでは name を addressに入れて運用
      propertyAddress: form.name.trim(),
      code: "P",
      expectedRent: parseYen(form.assumedRentYen),
      agentRent: parseYen(form.customerRentYen),
      expectedSalePrice: parseYen(form.expectedSalePriceYen),
      propertyPrice: parseYen(form.propertyPriceYen),
      expenses,
    };
    ...
    try {
      ...
      if (mode === "edit" && editingId) {
        saved = await apiUpdateProject(editingId, payload);
      } else {
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans text-slate-600 bg-slate-50">
      ...
      >
        {/* ヘッダー(h-28)と下端を合わせる：サイドの白帯も h-28 */}
        ...
        <div className="bg-white h-28 border-b border-slate-100 flex items-center">
          <header className="w-full px-8 flex items-center justify-end h-full">
            <div className="inline-flex items-center rounded-full border border-slate-200 bg-white shadow-sm overflow-hidden my-auto">
              ...
              <button
                onClick={openCreate}
                className="group w-12 h-11 inline-flex items-center justify-center font-extrabold text-[#FD9D24] hover:text-white transition hover:bg-[#FD9D24]"
                title="追加"
                aria-label="追加"
              >
                +
              ...
              <button
                ...
                disabled={!selectedId}
                className={[
                  "px-4 h-11 inline-flex items-center font-semibold transition",
                      ...
                      <div className="rounded-xl border border-slate-200 p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <button
                            ...
                            className="group inline-flex items-center justify-center rounded-full w-10 h-10 border border-slate-200 bg-white font-extrabold text-[#FD9D24] transition hover:bg-[#FD9D24] hover:text-white"
                            aria-label="内訳を追加"
                            title="内訳を追加"
                          >
                            +
                              ...
                              <div key={item.id} className="flex gap-2">
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setForm((prev) => {
                                      const updated = prev.buyCostItems.map((x) =>
                                        x.id === item.id ? { ...x, label: nextLabel } : x
                                      );
                                ...
                                <input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setForm((prev) => {
                                      const updated = prev.buyCostItems.map((x) =>
                                        x.id === item.id ? { ...x, amount: nextAmt } : x
                                      );
                                      return { ...prev, buyCostItems: ensureTrailingEmptyCostRow(updated) };
                                    });
                                  }}
                        ...
                        <div className="mt-3 flex items-center justify-between text-sm">
                          ...
                          <div className="font-semibold text-slate-800">
                            {formatYen(
                              form.buyCostItems.reduce((s, it) => s + (parseYen(it.amount) ?? 0), 0)
                            )}
                          </div>
                        </div>
                      </div>
